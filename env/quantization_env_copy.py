...
class QuantizationEnv:
    ...
    def _quantize_model(self, strategy):
        ...
        for m in self.model.modules():
            if type(m) in self.quantizable_layer_types:
                bits = strategy[-1]
                if bits != 32: # do not quantize if it is already 32
                    m.weight.data = self._quantize_tensor(m.weight.data, bits)
                # strategy_index += 1

    def _quantize_layer(self, layer_index, bitwidth):
    ...
    def step(self, action):
        ...
        self._quantize_layer(self.quantizable_idx[self.cur_ind], action)
        ...
        if self._is_final_layer():
            ...
            current_size = self._calculate_quantized_model_size()
            acc_t1 = time.time()
            ...
            if reward > self.best_reward:
                ...
                prGreen('New best reward: {:.4f}, acc: {:.4f}, compress: {:.4f}'.format(self.best_reward, acc, compression_ratio))
    ...
    def _cur_reduced(self, strategy):
        ...
        # print(self.strategy)
        quantized_size = self._calculate_model_size()
